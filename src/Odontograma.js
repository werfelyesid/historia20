...
useEffect(() => {
  const fetchIds = async () => {
      try {
        ...
        if (doctorId) {
          ...
          // Cambiamos a la ruta correcta dentro de 'users/{userId}/doctors'
          const doctorDocRef = doc(db, `doctor/${doctor.uid}/doctors`, 'default'); // Aquí 'default' o el ID del doctor correcto
          ...
          if (doctorDoc.exists()) {
            ...
            if (doctorData.doctorId && doctorData.patientId) {
              setDoctorId(doctorData.doctorId);
              setPatientId(doctorData.patientId);
              setIsLoading(false); // Cambia el estado de carga a false una vez que se obtienen los valores
            } else {
              console.error("Los datos no contienen doctorId o patientId.");
        ...
        console.error('Error al obtener doctorId y patientId:', error);
        setIsLoading(false);
      }
    };

    // Llamada a la función para obtener doctorId y patientId
    fetchIds();
  }, [auth]);

  // Precargar los datos cuando ya tenemos doctorId y patientId
  useEffect(() => {
    if (!doctorId || !patientId) {
      console.error('doctorId o patientId no están definidos');
      return;
    }

    const precargarDatos = async () => {
      try {
        // Ahora accedemos a la ruta correcta dentro de 'users/{userId}/doctors/{doctorId}/pacientes/{patientId}'
        const diagnosticosSnapshot = await getDocs(collection(db, `users/${auth.currentUser.uid}/doctors/${doctorId}/pacientes/${patientId}/diagnosticos`));
        const tratamientosSnapshot = await getDocs(collection(db, `users/${auth.currentUser.uid}/doctors/${doctorId}/pacientes/${patientId}/hechoPorHacer`));
  ...
  const agregarNotaAEvolucion = useCallback(async (dienteId, codigoTratamiento, nota) => {
    try {
      const evolucionDocRef = doc(db, 'doctors', doctorId, 'pacientes', patientId, 'evolucion', `${dienteId}-${codigoTratamiento}`);
  ...
  const applyDiagnostico = useCallback(async (dienteId, superficie, tipoDiagnostico) => {
    ...
    try {
      ...
      const diagnosticoDocRef = doc(db, 'doctors', doctorId, 'pacientes', patientId, 'diagnosticos', diagnosticoDocId);
  ...
  }, [doctorId, patientId, selectedDiagnostico]);
  ...
  const applyTreatment = useCallback(async (dienteId, superficie) => {
    ...
    if (element) {
      ...
      try {
        const odontogramaDocRef = doc(db, 'doctors', doctorId, 'pacientes', patientId, 'odontograma', elementId);
  ...
  }, [doctorId, patientId, selectedTratamiento, precios]);
