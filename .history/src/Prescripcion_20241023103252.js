...
const Prescription = ({ doctorId, patientId }) => {
        ...
        try {
          ...
          if (patientDoc.exists()) {
            ...
            setFormData((prevData) => ({
              ...prevData,
              nombrePaciente: `${patientData.primerNombre} ${patientData.primerApellido}`,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const prescriptionDocRef = doc(db, 'doctors', doctorId, 'pacientes', patientId, 'prescripciones', timestamp);
      ...
      const resumen = `Fecha: ${formData.fecha}, Hora: ${timestamp.split('T')[1].split('.')[0]}, Medicamento: ${formData.nombreMedicamento}, Presentación: ${formData.presentacionDosis}, Dosis: ${formData.dosis}, Frecuencia: ${formData.frecuencia}`;
  
      const evolucionesCollectionRef = collection(db, 'doctors', doctorId, 'pacientes', patientId, 'evoluciones');
