...
interface HistoriaClinicaOralProps {
  doctorUid: string;
  patientUid: string;
  onClose: () => void;
}

const HistoriaClinicaOral: React.FC<HistoriaClinicaOralProps> = ({ doctorUid, patientUid, onClose }) => {
  const initialValues = {
    enfermedadActual: '',
  ...
  const handleSubmit = async (values: typeof initialValues, { setSubmitting }: { setSubmitting: (isSubmitting: boolean) => void }) => {
    const docRef = doc(db, 'doctors', doctorUid, 'pacientes', patientUid);
