...
const EvolucionDelPaciente = ({ doctorId, patientId }) => {
  ...
  const loadEvolucion = useCallback(async () => {
    try {
      const evolucionSnapshot = await getDocs(collection(db, 'doctors', doctorId, 'pacientes', patientId, 'evolucion'));
  ...
  const agregarNotaAEvolucion = async (tratamientoId, nota) => {
    try {
      const evolucionDocRef = doc(db, 'doctors', doctorId, 'pacientes', patientId, 'evolucion', tratamientoId);
