...
function ConsentimientoOperatoria({ doctor, patient }) {
  ...
  useEffect(() => {
    ...
    getToothNumber(); // Ejecutar prompt para pedir número de diente cuando se monta el componente
  ...
  return (
    <>
      ...
      <div id="consentimiento-print" className="consentimiento-form" ref={consentimientoRef}>
        ...
        <p style={normalTextStyle}>
          Se me ha informado de la importancia de seguir todas las recomendaciones postoperatorias para asegurar la durabilidad de la restauración. Esto incluye mantener una buena higiene oral, asistir a las citas de control, y reportar cualquier anomalía o dolor persistente al Dr. {doctor?.name || 'N/A'}.
        </p>

        <h2>Alternativas al Tratamiento:</h2>
        <p style={normalTextStyle}>
          El Dr. {doctor?.name || 'N/A'} me ha explicado las alternativas al procedimiento de operatoria dental, que incluyen no tratar el diente afectado y monitorear su estado, lo cual podría conllevar riesgos como la progresión de la caries, infecciones, problemas estéticos, o la necesidad de procedimientos más invasivos en el futuro.
        </p>

        <h2>Consentimiento y Autorización:</h2>
        <p style={normalTextStyle}>
          He recibido una explicación clara y comprensible sobre los riesgos, beneficios y posibles complicaciones del procedimiento de operatoria dental. Acepto voluntariamente someterme a este procedimiento y autorizo al Dr. {doctor?.name || 'N/A'} a realizar cualquier intervención adicional que sea necesaria para tratar cualquier complicación que pudiera surgir.
        </p>
